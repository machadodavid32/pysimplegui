...
from utilidades_threads import baixar_arquivo, logar
...
layout = [
    ...
    [sg.Button('Logar', key='botao_logar')],
...
while True:
    ...
    elif event== 'botao_logar':   # Será conectado a uma def em utilidades_threads
        thread_login = Thread(target=logar, args=(window, values), daemon=True)   
        thread_login.start()
        print('Iniciando login') 
        window['botao_logar'].update(disabled=True)
    elif event == 'login_finalizado':
        thread_login.join()
        window['botao_logar'].update(disabled=False)
        print(values['login_finalizado'])   
